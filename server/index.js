...
app.post("/adddata", (req, res) => {
  ...
  while (rndid in keys) rndid = parseInt(1 + Math.random() * 10000, 10);
  let data = {
    id: rndid,
    task: msg,
    done: false,
  ...
  // console.log(keys);
  res.send(
    JSON.stringify({ status: 200, error: null, response: "Added successfully" })
  );
});
...
app.delete("/remove/:id", (req, res) => {
  ...
  for (var ky in keys) if (keys[ky] >= ind) keys[ky]--;
  // console.log(keys);
  // taskdata[ind].done = req.body.chk;
  res.send(
    JSON.stringify({ status: 200, error: null, response: "Added successfully" })
